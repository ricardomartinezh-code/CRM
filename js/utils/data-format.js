(function(global, factory){
  ...
  function formatLosgList(value){
    ...
    if(Array.isArray(value)){
      return value.map(item => formatLosgList(item)).filter(Boolean).join(' | ');
    }
    if(value instanceof Date){
      return formatLosgDateTime(value);
    }
    if(typeof value === 'object'){
      const name = cleanValue(value.name || value.nombre || value.displayName || value.usuario || value.user || value.title);
      const email = cleanValue(value.email || value.correo || value.mail);
      const role = cleanValue(value.role || value.rol || value.perfil);
      const descriptor = cleanValue(value.descripcion || value.description || value.detalle || value.detail || value.etiqueta || value.label || value.tipo || value.action || value.accion || value.resumen);
      const target = cleanValue(value.destino || value.target || value.asignado || value.asignadoA || value.para || value.to || value.recipient);
      const parts = [];
  ...
  function formatLosgAssignments(value){
    if(value === null || value === undefined) return '';
    if(Array.isArray(value)){
      return value.map(item => formatLosgAssignments(item)).filter(Boolean).join(' | ');
    }
    if(typeof value === 'object'){
      const permiso = cleanValue(value.permiso || value.permission || value.tipo || value.scope || value.descripcion || value.description);
      const destinatario = cleanValue(value.usuario || value.user || value.destino || value.target || value.asignado || value.asignadoA || value.para || value.to || value.nombre);
      const detalle = cleanValue(value.detalle || value.detail || value.resumen || value.notes || value.resultado);
      const parts = [];
  ...
  function normalizeLosgEntry(raw){
    ...
    let deviceSummary = formatLosgList(pick('dispositivos','devices','device_types','tipodispositivo','tiposdispositivo','origenes'));
    if(!deviceSummary){
      const pieces = [];
    ...
    const normalized = {
      ...
      primer_acceso: formatLosgDateTime(pick('primeracceso','firstlogin','primeravez','first_seen','inicio','primer_ingreso','primeraconexion')),
      ultimo_acceso: formatLosgDateTime(pick('ultimoacceso','lastlogin','ultimavez','last_seen','final','ultimo_ingreso','ultimaconexion')),
      ...
      importaciones: formatLosgNumber(pick('importaciones','imports','import_count','accionesimportacion','importactivity')),
      exportaciones: formatLosgNumber(pick('exportaciones','exports','export_count','accionesexportacion','exportactivity')),
      cambios_masivos: formatLosgNumber(pick('cambiosmasivos','massupdates','bulkupdates','bulk','bulk_count','movimientosmasivos','actualizacionesmasivas')),
      diagnosticos: formatLosgNumber(pick('diagnosticos','diagnostics','diagnostics_count','ejecucionesdiagnostico','diagnosticosrealizados')),
      resultado_diagnostico: formatLosgList(pick('resultadodiagnostico','diagnostics_result','diagnosticosresultado','diagnosticos_resumen','diagnosticosdetalle','diagnosticresult')),
      permisos_asignados: formatLosgAssignments(pick('permisosasignados','permisos','permissions','permission_changes','permission_assignments')),
      planteles_asignados: formatLosgAssignments(pick('plantelasignado','planteles','campus','planteles_asignados','campus_assignments'))
    };
